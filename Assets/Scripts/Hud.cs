    ...
    void LateUpdate()
    {
        switch (type)
        {
            case InfoType.Exp:
                ...
                //float max_exp = GameManager.nextExp[GameManager.instance.level];
                //my_slider.value = current_exp / max_exp;
                break;
            
            case InfoType.Gold:
                //my_text.text = string.Format("G {0:N0}", GameManager.instance.gold);
                break;
                // 포맷 방식에서 N: 천단위 콤마 자동 추가/ 0: 소수점 없음
            
            case InfoType.Level:
                //my_text.text = string.Format("Lv.{0:F0}", GameManager.instance.level);
                break;
            ...
            case InfoType.Health:
                // float current_health = GameManager.instance.health;
                // float max_health = GameManager.instance.max_health;
